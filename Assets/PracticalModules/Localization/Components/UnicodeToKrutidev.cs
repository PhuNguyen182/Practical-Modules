	...
	public static string UnicodeToKrutiDev(string unicode_substring)
	{
		...
		while (position_of_f != -1) //while-02
		{
			...
			while ((position_of_f != 0) && (modified_substring[position_of_f - 1] == '्'))
			{
				...
				modified_substring =
					modified_substring.Replace(string_to_be_Replaced + "f", "f" + string_to_be_Replaced);

				position_of_f = position_of_f - 2;
			}

			position_of_f =
				modified_substring.IndexOf("ि", position_of_f + 1); // search for f ahead of the current position.
		...
		while (position_of_half_R > 0) // while-04
		{
			...
			var string_to_be_Replaced = modified_substring.Substring(position_of_half_R + 2,
		...
		for (int input_symbol_idx = 0; input_symbol_idx < array_one_length; input_symbol_idx++)
		{
			...
			{
				modified_substring =
					modified_substring.Replace(array_one[input_symbol_idx], array_two[input_symbol_idx]);
